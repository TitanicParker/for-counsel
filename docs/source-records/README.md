# Source Records

This folder contains redacted source documents prepared for publication as part of the counsel-facing case site.

These files are evidential materials. They are not argument pages, submissions, or narrative essays. Their purpose is to preserve the underlying documentary record in a clean, stable, and inspectable form.

## Handling rules

1. Do not rewrite source text for style.
2. Do not add analysis to source-record pages.
3. Preserve redactions, OCR irregularities, pagination artefacts, and documentary sequence unless a correction is explicitly logged.
4. Each source-record page should identify itself as a redacted source document.
5. Each source-record page should state that no analytical findings are made on that page.
6. Interpretive pages elsewhere in the site may link to these records, but should not merge analysis into them.
7. A transcript is not a substitute for the native source. Where the repository holds a transcript rather than the original binary file, the source page must say so explicitly.

For the current acquisition/completeness position see [`../../evidence-audit/source-acquisition-status.md`](../../evidence-audit/source-acquisition-status.md).

## Clinical source corpus

- `gp-referral.html`
- `discharge-summary.html`
- `neurology-notes-2017.html`
- `neurology-notes-2018.html`
- `neurology-notes-2019.html`
- `neurology-notes-2020.html`
- `neurology-notes-2021.html`
- `neurology-notes-2022.html`
- `neurology-notes-2023.html`
- `neurology-notes-2024.html`
- `neurology-notes-2025.html`
- `official-rebuttal.html`

## Materialised correspondence / governance / later specialist sources

- `gmail-2017-10-24-medical-assessment.md` — contemporaneous patient email recording medication withdrawal/reintroduction, incomplete symptom control at the lower dose, podiatry appointment and major physical improvement.
- `clinician-to-healy-2017-myself-and-tim.md` — reproduced contemporaneous Monaghan-to-Healy correspondence containing “Myself and Tim are keen to hold back on Levodopa...” and requesting genetic/second-opinion advice; exact date and identity of “Tim” are not self-proving from the reproduced page.
- `complaint-2025-03-19.md` — redacted transcript of the attached formal complaint submitted on 19 March 2025.
- `qps-letter-2025-04-15.md` — QPS complaint-routing letter recording that Sinemet CR had been an effective analgesic for foot pain and that the complaint was routed as clinical judgment.
- `counihan-response-2025-05-07.md` — signed clinician response stating “As I have not been involved in your care” and forwarding the concerns to Dr Monaghan.
- `qps-imf-par-letter-2025-05-30.md` — QPS letter stating that, because the patient remained dissatisfied, the complaint would be managed through the Incident Management Framework, a Preliminary Assessment Report was being completed, and the PAR would be reviewed at an IHA West and North West SIMT meeting.
- `gdpr-article16-2025-06-12.md` — redacted transcript of the Article 16 rectification/annotation request concerning clinical records.
- `qps-imf-letter-2025-06-13.md` — two-page QPS process letter stating that the hospital had moved the complaint to an independent Incident Management Framework review and distinguishing that route from ordinary YSYS clinical-judgment complaint handling.
- `gdpr-article16-qps-2025-06-15.md` — separate Article 16 request concerning QPS closure-letter material and the 15 April QPS letter.
- `gdpr-article16-reminders-2025-07-07.md` — two separate reminder emails concerning the 12 June clinical-record request and 15 June QPS request.
- `gdpr-article15-request-2025-07-07.md` — Article 15 access request concerning the reported July 2017 bedside teaching interaction and related trainee/ward/governance records.
- `gdpr-acknowledgment-clarification-2025-07-14.md` — GUH acknowledgment of a rectification request and the same-day data-subject request to identify which pending GDPR request was being acknowledged.
- `gdpr-article16-consolidated-2025-07-16.md` — consolidated/final Article 16 correspondence and attached argument; retained as evidence of the propositions being advanced at that date, not as findings.
- `patient-imf-clarification-2025-07-18.md` — patient request, copied to the Patient Advocacy Service, seeking clarification of whether the PAR had been discussed, what review was underway and what process/timeline applied.
- `medical-council-monaghan-response-2025-07-29.md` — Dr Monaghan's later regulatory submission, including his account of Prof Counihan's 2017 advice at a Neurology Weekly Complex Case meeting.
- `monaghan-supporting-documentation-2025-07-29.md` — source-bundle index for the 185-page redacted supporting-documentation PDF supplied by Dr Monaghan; identifies reproduced 2017–2025 clinical material and keeps later repetition separate from independent historical corroboration.
- `medical-council-counihan-decision-2025-07-29.md` — Medical Council CEO decision using the narrower wording that Dr Counihan was “not directly involved in your care.”
- `gdpr-article18-2025-08-04.md` — transcript of the Article 18 restriction notice sent to GUH/HSE DPO and QPS.
- `gdpr-article15-decision-2025-08-05.md` — GUH Article 15 search/non-holding decision concerning bedside teaching/student/ward-round records and routing podiatry records to Community Healthcare West.
- `par-status-2025-08-06-via-patient-advocacy.md` — Patient Advocacy Service email reproducing the hospital's 6 August statement that the PAR was being prepared for August SIMT discussion; derivative route to the hospital communication.
- `simt-outcome-signed-2025-09-01.md` — photographed signed SIMT outcome letter dated 1 September 2025, reporting the 27 August decision.
- `simt-outcome-2025-09-03.md` — separate two-page SIMT outcome PDF dated 3 September 2025. It is retained separately because its date/wording are not identical to the signed 1 September artefact.
- `gdpr-article16-decision-clinical-2025-09-06.md` — separate Article 16 decision refusing the 12 June request concerning clinical-record rectification/annotation and explaining the controller's historical-opinion/continuity rationale.
- `gdpr-article16-decision-qps-2025-09-06.md` — separate Article 16 decision refusing the 15 June request concerning QPS closure letters on the stated basis that the requested material was not personal data within the scope asserted.
- `dpc-reengagement-2026-01-13.md` — DPC correspondence confirming CAERU handling and suggesting renewed engagement with UHG.
- `gdpr-renewed-article16-18-2026-01-14.md` — renewed Article 16/18 request following DPC correspondence.
- `qps-response-2026-01-26.md` — transcript of the January 2026 QPS governance response.
- `gdpr-dpo-clarification-2026-01-27.md` — data-subject clarification expressly distinguishing the QPS governance position from any later GDPR determination by the DPO.
- `genetics-prkn-2026-02-04.md` — redacted transcript of the St Vincent's specialist genetic letter stating that Parkinsonism and dystonia are due to compound heterozygous Parkin mutations.
- `dpc-notification-no-guh-dpo-response-2026-02-16.md` — notification to the DPC recording the data subject's position that the renewed January 2026 Article 16/18 engagement had received no acknowledgment, extension notice or substantive DPO response by 16 February.

## Source-status caution

Some entries above are faithful redacted transcriptions made from original Gmail messages, attachments, PDFs, photographed letters or complete source texts directly supplied to and inspected in the project. The original message, attachment, PDF or image remains the controlling source. Where practical, a properly redacted native copy should also be preserved in the permanent library.

A later source may be primary evidence of what was said or decided at that later date while remaining derivative as proof of an earlier historical clinical event. In particular, the reproduced 2017 `Myself and Tim` page, the 2025 Counihan letter, Monaghan submission, Monaghan supporting-documentation bundle and Medical Council decision must remain separate evidential origins. The 2017 page proves its own wording but does not by itself identify “Tim”; the later Monaghan submission supplies that identification/account but remains retrospective. The 185-page July 2025 bundle shows what Dr Monaghan retained and supplied in 2025; reproduced historical passages inside it are not counted as new independent corroboration of those earlier events.

The two 6 September 2025 Article 16 decisions are also separate documents addressing separate requests. They should not be collapsed into a single “rectification refusal.”

The 30 May and 13 June 2025 QPS letters are evidence of the hospital's represented process architecture — including a PAR, prospective SIMT review and an assurance of independent IMF review — but not proof of what evidence the later PAR/SIMT actually used or whether proposition-level independent revalidation occurred. The 18 July patient clarification request is evidence of what process information remained unclear to the patient, not proof of the internal state.

The 26 January 2026 QPS correspondence is a governance position, not a GDPR determination. The 27 January source records that distinction expressly. The 16 February DPC notification records an asserted non-response position; it does not by itself determine whether a legal breach occurred.

## Publication status

This directory is designed for GitHub Pages publication. The source documents should remain separate from counsel-facing analysis so that readers can distinguish primary record from interpretation.